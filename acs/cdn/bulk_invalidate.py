...
Examples:
    acs/cdn/bulk_invalidate.py --root http://foo.bar.com/
    acs/cdn/bulk_invalidate.py --root https://foo.bar.com/
...
parser.add_argument(
    '--root', metavar="ROOT", type=str, dest='root',
    action='store', help='Invalidates all objects from the root. Note that the http(s) and the ending "/"')
parser.add_argument(
    '--file', metavar="FILE", type=str, dest='file',
...
if not root and not file:
    print 'At least root or file must be specified'
    exit(1)
...
if len(data["dirs"]) > 0:
    ...
    try:
        ...
        print "Failed to invalidate dirs"
        print e